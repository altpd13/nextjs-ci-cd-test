...
export function suiTypeParameterName(
  idx: number,
  typeParameter: SuiTypeParameter
) {
...
export function suiTypeName(
  ...
  console.info(`parameterType`, JSON.stringify(parameterType, null, 2));
    ...
    if (t.Struct.typeArguments.length > 0) {
      const typeArgs = t.Struct.typeArguments.map((targ: any) => {
        if (typeof targ.TypeParameter === "number") {
          return `T${targ.TypeParameter}: ${typeParameters?.[
            targ.TypeParameter
          ].abilities.join(" + ")}`;
        }
  ...
  throw new Error(
    `suiTypeName Invalid type ${JSON.stringify(parameterType, null, 2)}`
  );
}

export function stringifySuiVectorType(
  t: any,
  typeParameters?: SuiMoveAbilitySet[]
): string {
  if (!t.Vector) {
