...
import { isHexadecimal } from "../utils/sui-parser";
import { SuiFunc } from "../utils/sui-types";
import { parseArgVal } from "../utils/sui-helper";
...
const VectorArgForm: React.FunctionComponent<Props> = ({
  ...
  // );
  const [u8vecParseType, setU8VecParseType] =
    useState<U8VecElParseType>("string");
  const [args, setArgs] = useState<any[]>([]);
  const [u8VecHexParseCtxs, setU8VecHexParseCtxs] = useState<
    U8VecHexParseCtx[]
  >([]);
  ...
  const handleFormChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (typeName === "Vector<U8>" && u8vecParseType !== "decimal") {
      if (u8vecParseType === "string") {
        const hexCtx = u8VecHexParseCtxs.find(
          (ctx) => ctx.idx === indices.toString()
        );
        if (!hexCtx) {
      ...
      if (id.includes("bool")) {
        ...
        if (vectorElType === "U8" && u8vecParseType !== "decimal") {
          if (u8vecParseType === "string") {
            ...
            const arr = Array.from(Buffer.from(value, "utf8"));
            data[indices[0]] = arr.map((a) =>
              parseArgVal(a, vectorElType, u8vecParseType)
            );
            setArgs(data);
            ...
            setU8VecHexParseCtxs([...u8VecHexParseCtxs]);
            console.log(
              `handleFormChange hex u8VecHexParseCtxs`,
              u8VecHexParseCtxs
            );
            if (isHexadecimal(value)) {
              const arr = Array.from(Buffer.from(value, "hex"));
  ...
  const parseU8Vector = (nums: number[], indexMemo: number[]) => {
    ...
    const hexStr =
      u8VecHexParseCtxs.find((ctx) => ctx.idx === indexMemo.toString())?.hex ||
      "";
    ...
    if (!Array.isArray(val)) {
      ...
      if (vectorElType === "Bool") {
        ...
        return (
          ...
          >
            <div
              style={{ display: "flex", alignItems: "center", height: "1em" }}
            >
              <input
